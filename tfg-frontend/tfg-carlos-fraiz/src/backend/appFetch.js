...
export const configFile = (method, file, nombre, descripcion) => {
    ...
    config.body.append('file', file, "input.xml");
    config.body.append('nombre', nombre);
    config.body.append('descripcion', descripcion); 
     

    let serviceToken = getServiceToken();

       
    config.headers.append("Authorization", `Bearer ${serviceToken}`);
