...
import es.udc.tfg.backend.rest.controllers.common.JwtInfo;
...
public class UserController {
	...
	public ResponseEntity<AuthenticatedUserDto> signUp(
		...
		if (!userDto.getSecondLastName().isBlank())
			user.setLastName2(userDto.getSecondLastName());
	...
	public UserDto updateProfile(@RequestAttribute Long userId, @PathVariable("id") Long id,
		...
		return toUserDto(userService.updateProfile(id, userDto.getFirstName(), userDto.getFirstLastName(),
				userDto.getSecondLastName(), userDto.getEmailAdress()));
	...
	private String generateServiceToken(User user) {
		...
		JwtInfo jwtInfo = new JwtInfo(user.getId(), user.getUserName(), user.getRole().getName());
